...
const CartDetails = () => {
  ...
  return (
    ...
    <h5 className="text-light" >Cart Calculation {carts.length > 0 ? `(${carts.length})` : ''}</h5>

    {
        carts.length > 0 ? <button className="btn btn-danger" onClick={emptyCart} ><i className="far fa-trash-alt" ></i> Empty Cart</button>:''
    ...
    <div className="card-body p-0" >
    {
        ...
        <th>Price</th>
        <th>Qty</th>
        <th className="text-right" ><span className="amount" id="amount" >Total Amount</span></th>
        </tr>
        </thead>
...

        {
            carts.map((el) => {
                return (
                    ...
                    <td>
                    <button className="prdct-delete" onClick={() => handleDecrement(el.id)} ><i className="far fa-trash-alt" ></i></button>
                    </td>
                    ...
                    <td>
                    <div className="prdct-qty-container" >
                    <button className="prdct-qty-btn" onClick={el.qnty <= 1 ? () => handleDecrement(el.id) : () => handleSingleDecrement(el)} ><i className="fa fa-minus" ></i></button>
                    <input type="text" className="qty-input-box" disabled value={el.qnty} />
                    <button className="prdct-qty-btn" onClick={() => handleIncrement(el)} ><i className="fa fa-plus" ></i></button>
                    </div>
                    </td>
            
                    <td className="text-right" >{el.qnty * el.price}</td>
